...
function ProductPage() {
  ...
  useEffect(() => {
    ...
    if (savedVariant && productId && productId === savedVariant.id) {
      setSelectedVariant(savedVariant);
    }
    if (savedCep) {
      setCepInput(savedCep);
      fetchAddress(savedCep.replace('-', ''));
    }
  }, [cookies]);
  ...
  useEffect(() => {
    if (selectedVariant?.size || selectedVariant?.color) {
      setCookieWithExpiry('selectedVariant', {
        ...selectedVariant,
